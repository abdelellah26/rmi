...
public class ChatClient extends UnicastRemoteObject implements InterfaceClient{
    ...
    //constructeur
    public ChatClient(String name ,String email,String password, InterfaceServer server,JTextArea jtext1,JTextArea jtext2,JPanel jpanel) throws RemoteException{
        this.name = name;
        this.email=email;
     ...
     */
    @Override
    public void getM() throws RemoteException{
                   Object[][] messages = server.getMessages(email);

            // Afficher les messages
            for (Object[] messageInfo : messages) {
                        output.setText(output.getText() + "\n" +messageInfo[1]+" : " + messageInfo[0]);
            }
    }
    ...
    public void sendMessage(List<String> list) {
        try {
           server.broadcastM(input.getText(), email, list);
           server.broadcastMessage(email +" : "+input.getText(),list);
    ...
    public void blockClient(List<String> clients) throws RemoteException {
        // Préparation de la requête SQL pour insérer les utilisateurs bloqués dans la table blocked_users
        server.blockClient(email,clients);
    ...
    public String getName() {
        ...
        //cette fonction pour recupere le email d'un client connectées
    @Override
    public String getEmail() {
        return email;
    }
        //cette fonction pour recupere le password d'un client connectées
    @Override
